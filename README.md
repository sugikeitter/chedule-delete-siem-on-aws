...
```bash
schedule-delete-siem-on-aws$ sam build --use-container
```
...
```bash
schedule-delete-siem-on-aws$ sam logs -n StockCheckerFunction --stack-name schedule-delete-siem-on-aws --tail
```
...
```bash
schedule-delete-siem-on-aws$ pip install -r tests/requirements.txt --user
# unit test
schedule-delete-siem-on-aws$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
schedule-delete-siem-on-aws$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name schedule-delete-siem-on-aws
```
